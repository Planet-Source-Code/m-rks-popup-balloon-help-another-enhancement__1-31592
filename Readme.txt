VB Popup Balloons (Release 1.0)
By, Robert Morris
http://rmsoft.itgo.com

'Updated by:
'              Todd Porter (aka: CodeJack) (Release 2.0, 2/5/2002)
'              http://www.2morrow.com
'
'- Rewrote the routines for creating the rounded rectangle - now all
'  done in 1 line of code!
'- changed the border to be drawn using the API instead of using the
'  shape control.
'- Added code to make everything move around dynamically to make
'  it resizable.
'- Added a caption to the balloon and made it so it could
'  be moved around by the caption
'- Added RTF support to be able to display RTF text.

'Updated by:
'		Mark Sollitt (markS) (Release 2.1, 8/2/2002)
'
'- Added the ability to display a command button. Text will not over-run
'  the button. The button can be used for any normal button function. '  Used the LaVolpeButton User Control due to the Button styles available

-THIS EXPLAINS WHAT THIS SOURCE CODE DOES AND A LITTLE BIT ABOUT
...
The form frmTip is a "template" form. We won't use it directly; every
time we want a balloon, we'll create a new instance of it, as in:
   Dim frmMyBubble as New frmTip
...
That's just an example. See the code's comments (frmTip, go to (General),
